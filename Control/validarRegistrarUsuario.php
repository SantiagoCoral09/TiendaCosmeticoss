...
if(empty($_POST["pass"]) || empty($_POST["login"]) || empty($_POST["nombre"]) || empty($_POST["apellido"]) || empty($_POST["cc"]) || empty($_POST["Ncelular"])){
...
echo ' pass: '.$pass.' user: '.$user.' nombre: '.$nombre.' apellido: '.$apellido.' cc: '.$cc.' Ncelular: '.$Ncelular;
if($rol==1){////si es un admin para que lo redirija a la pagina del administrador
    ...
    if ($existe==1)  ////si existe un registro en la base de datos con el usuario ingresado
    {
        ...
        $registrar="INSERT INTO usuarios(identificacion, nombres, apellidos, celular, correo, password, rol) VALUES ('$cc','$nombre','$apellido','$Ncelular','$user','$pass','$rol')";
    ...
    if ($existe==1)  ////si existe un registro en la base de datos con el usuario ingresado
    {
        ...
        $registro="INSERT INTO `usuarios`(`identificacion`, `nombres`, `apellidos`, `celular`, `correo`, `password`, `rol`) VALUES ('$cc','$nombre','$apellido','$Ncelular','$user','$pass','$rol')";
        print_r($registro);
        // $registrar="INSERT INTO usuarios(identificacion, nombres, apellidos, Ncelular, correo, password, rol) VALUES ('$cc','$nombre','$apellido','$Ncelular','$user','$pass','$rol')";
    
        $resultadoRegistro=$conexion->query($registro);
        ...
        echo '<br> pass: '.$pass.' user: '.$user.' nombre: '.$nombre.' apellido: '.$apellido.' cc: '.$cc.' Ncelular: '.$Ncelular;
    }
    mysqli_close($conexion);  // Cierra la conexion
